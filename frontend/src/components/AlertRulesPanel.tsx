      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-3xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
          >
            {/* Header */}
            <div className="px-6 py-4 flex items-center justify-between border-b border-gray-200 dark:border-white/5 bg-white dark:bg-zinc-900/50">
              ...
              <button onClick={onClose} className="p-2 text-gray-600 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              {/* Existing Rules List */}
              <div className="mb-8">
                <h3 className="text-sm font-semibold text-gray-600 dark:text-zinc-400 uppercase tracking-wider mb-4">{t('alerts.rules.configured')}</h3>
                {rules.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 dark:text-zinc-500 bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-xl">
                  ...
                  <div className="space-y-3">
                    {rules.map((rule) => (
                      <div
                        key={rule.id}
                        className="bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-xl p-4 flex items-center justify-between hover:border-gray-300 dark:hover:border-zinc-700 transition-colors"
                      >
                            ...
                            <span
                              className={`px-2 py-0.5 text-xs font-medium rounded-md ${
                                rule.enabled
                                  ...
                                  : 'bg-gray-200 dark:bg-zinc-700/50 text-gray-600 dark:text-zinc-500 border border-gray-300 dark:border-zinc-700'
                              }`}
                            >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="p-2 text-indigo-500 dark:text-indigo-400 hover:bg-indigo-500/10 rounded-lg transition-colors"
                          >
                            <Edit2 size={16} />
                          </button>
                          <button
                            ...
                            disabled={loading}
                            className="p-2 text-rose-500 dark:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors disabled:opacity-50"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
              ...
              <div className="pt-6 border-t border-gray-200 dark:border-zinc-800/50">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-zinc-500 mb-1">{t('alerts.rules.ruleName')}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 dark:text-zinc-500 mb-1">{t('alerts.rules.ruleType')}</label>
                    <select
                      value={formType}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-gray-600 dark:text-zinc-500 mb-1">{getThresholdLabel(formType)}</label>
                      <input
                        type="number"
                        value={formThreshold}
                    ...
                    <div>
                      <label className="block text-sm text-gray-600 dark:text-zinc-500 mb-1">{t('alerts.rules.cooldown')}</label>
                      <input
                        type="number"
                        value={formCooldown}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <label htmlFor="enabled" className="text-sm text-gray-700 dark:text-zinc-300">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    {isEditing && (
                      <button
                        onClick={resetForm}
                        className="px-4 py-2.5 bg-gray-200 dark:bg-zinc-800 hover:bg-gray-300 dark:hover:bg-zinc-700 text-gray-900 dark:text-white rounded-lg transition-colors"
                      >
