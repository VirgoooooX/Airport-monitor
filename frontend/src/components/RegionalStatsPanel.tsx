  ...
  if (regionalStats.length === 0) {
    return (
      <div className="glass-panel p-8">
        <p className="text-zinc-500 text-center">No regional data available</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">Regional Statistics</h3>
            <p className="text-sm text-zinc-400">Node distribution and performance by region</p>
          </div>
        </div>
        ...
        {chartType === 'bar' ? (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={barChartData}>
              ...
              />
              <Bar dataKey="availability" fill="#6366f1" name="Availability (%)" />
        ...
        {regionalStats.map((stat, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-white font-medium capitalize">{stat.region}</h4>
            ...
            {stat.countries.length > 0 && (
              <div className="mt-3 pt-3 border-t border-zinc-800">
                <p className="text-xs text-zinc-500 mb-2">Top Countries:</p>
                <div className="space-y-1">
                  {stat.countries.slice(0, 3).map(country => (
                    <div key={country.country} className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-zinc-500">{country.avgAvailabilityRate.toFixed(0)}%</span>
                    </div>
                  ))}
                </div>
              </div>
